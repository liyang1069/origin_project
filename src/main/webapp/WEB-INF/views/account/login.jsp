                            ...
                            <fieldset>
								...
								if(error != null){
									if(error.endsWith("UnknownAccountException") || error.endsWith("IncorrectCredentialsException")){
										error = "zh".equalsIgnoreCase(request.getLocale().getLanguage())? "用户名或密码错误。":"Username or password error.";
									}
									if(error.endsWith("LockedAccountException")){
									    error = "zh".equalsIgnoreCase(request.getLocale().getLanguage())? "您的帐号已被锁定。<br/>您可以<a target='_blank' href='https://xmo.optimix.asia/zh-cn/admin/forget_password'>点击此处</a>以进行解锁":
                                        "Your account has been locked.<br/>You may <a target='_blank' href='https://xmo.optimix.asia/en/admin/forget_password'>click here to unlock</a>";
									}
								%>
